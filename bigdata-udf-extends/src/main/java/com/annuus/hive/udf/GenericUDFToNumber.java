...
public class GenericUDFToNumber extends GenericUDF {
	...
	public ObjectInspector initialize(ObjectInspector[] arguments) throws UDFArgumentException {
		...
		if (arguments.length > 1
				&& !(arguments[1].getTypeName() == serdeConstants.DOUBLE_TYPE_NAME ||
						arguments[1].getTypeName() == serdeConstants.STRING_TYPE_NAME)) {
			throw new UDFArgumentTypeException(1, "Argument " + (2) 
					+ " of function to_number must be \""
					+ serdeConstants.DOUBLE_TYPE_NAME + "\", but \""
	...
	public Object evaluate(DeferredObject[] arguments) throws HiveException {
		...
		try {
			...
			if (formatter instanceof DecimalFormat) {
				...
				if (returnInspector.getTypeName() == serdeConstants.BIGINT_TYPE_NAME) {
