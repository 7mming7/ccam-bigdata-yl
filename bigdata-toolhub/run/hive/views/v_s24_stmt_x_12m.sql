...
CREATE VIEW if not exists V_S24_STMT_X_12M AS
SELECT T1.BANK,
          T1.XACCOUNT,
          T1.MONTH_NBR,
          T1.PAYMENT,
          ...
          END
             AS FLAG_CON_ADD_AGE                                  ,
          CASE
             WHEN COAL (T1.ODUE_FLAG, 0) + COAL (T3.ODUE_FLAG, 0) >= 1
             THEN
                1
             ELSE
                0
     ...
     FROM MDM_STMT_X T1
          ...
          (  SELECT P.ACCOUNT AS XACCOUNT,
                    ...
                    SUM (
                       CASE
                          WHEN (P.PTLG_TYPE + 0) >= '10' AND (P.PTLG_TYPE + 0) <= '19'
                          THEN
                             POINT
                          ELSE
                             0
              ...
              WHERE     todate(P.CREATE_DATE,'yyyy-mm-dd') <= DATE_ADD(SYSDATE() ,1)
                    AND todate(P.CREATE_DATE,'yyyy-mm-dd') > ADD_MONTHS (SYSDATE(), -12)
          ...
          LEFT JOIN MDM_STMT_X T3
             ON     T1.XACCOUNT = T3.XACCOUNT
                ...
                AND todate(t3.STMT_DATE,'yyyy-mm-dd') > ADD_MONTHS (SYSDATE(), -13)
    WHERE     todate(t1.STMT_DATE,'yyyy-mm-dd') <= DATE_ADD(SYSDATE() ,1)
          AND todate(t1.STMT_DATE,'yyyy-mm-dd') > ADD_MONTHS (SYSDATE(), -12)
;
