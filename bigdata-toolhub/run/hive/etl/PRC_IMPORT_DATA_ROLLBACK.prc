...
CREATE OR REPLACE PROCEDURE PRC_IMPORT_DATA_ROLLBACK (IN db STRING, IN ETL_DATE STRING, IN cur_table_name STRING)
BEGIN
...
DBMS_OUTPUT.PUT_LINE('# ETL Date: '||ETL_DATE);
DBMS_OUTPUT.PUT_LINE('#');

 DECLARE table_column_cr CURSOR FOR 'desc '||db||'.'|| cur_table_name;                                       
 ...
 DECLARE table_info_cr CURSOR FOR
   'SELECT FDM_TABLE, SOURCE_TABLE, KEY_COL FROM '||db||'.T_FDM_TABLENAME WHERE CUR_DATA_TABLE =TRIM('' '||cur_table_name||''' )';
   OPEN table_info_cr;
     FETCH table_info_cr  INTO FDM_TABLE,SOURCE_TABLE,KEY_COL;
       WHILE SQLCODE=0 THEN
         ...
         v_sql := 'INSERT OVERWRITE TABLE '||db||'.'||cur_table_name||' SELECT p.* FROM '||db||'.'||cur_table_name||' p ';
         v_sql := v_sql ||' LEFT JOIN '||db||'.' ||FDM_TABLE|| ' t ON '|| v_join || ' and '
         ||'t.end_date=todate('|| ETL_DATE|| ', ''yyyymmdd'')'
		 ...
		 v_sql := ' ALTER TABLE ' ||db||'.'||FDM_TABLE|| ' DROP PARTITION (end_date='||ETL_DATE||') ';
		 DBMS_OUTPUT.put_line (v_sql);
		 EXECUTE IMMEDIATE v_sql;
